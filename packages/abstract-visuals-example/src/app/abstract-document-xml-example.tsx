...
async function genereteDoc(
  ...
  const [doc, _ignored_imageUrls, _ignored_fontFamilies] = ADXml.abstractDocXml(template, data, partials);
  const docWithResources = AD.AbstractDoc.addResources(doc, { imageResources });
  const blob: Blob =
    format === "PDF"
      ? await AbstractDocExporters.Pdf.exportToHTML5Blob(PDFDocument, docWithResources)
      : new Blob([await AbstractDocExporters.Docx.exportToHTML5Blob(docWithResources)], {
