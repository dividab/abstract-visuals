...
import { ImageResource } from "../../../abstract-document/src/abstract-document/index.js";
...
const wiringUrl1 = "./wiring1";
const wiringUrl2 = "./wiring2";
  ...
  const [template, setTemplate] = React.useState(`<AbstractDoc>
    ...
    <Section>
        <Table columnWidths="200,70,60, 60">
            ...
            <ImageRow src="${wiringUrl1}" height="100" width="300"/>
            <TableRow>
                {{> inlinePartial}}
            </TableRow>
            <ImageRow src="${wiringUrl2}" height="100" width="300"/>
          {{>partial}}
        </Table>
    </Section>
  </AbstractDoc>`);
...
async function generatePDF(
  ...
  const abstractImage = AI.createAbstractImage(AI.createPoint(0, 0), AI.createSize(200, 100), AI.white, [
    AI.createBinaryImage(AI.createPoint(0, 0), AI.createPoint(200, 100), "png", {
      type: "url",
      url: pngBase64,
    }),
  ]);
  const imageResources1: Record<string, ImageResource.ImageResource> = {
    [wiringUrl1]: ImageResource.create({ id: wiringUrl1, abstractImage }),
  };
  const imageResources2: Record<string, ImageResource.ImageResource> = {
    [wiringUrl2]: ImageResource.create({ id: wiringUrl2, abstractImage }),
  };
  const doc: AD.AbstractDoc.AbstractDoc = {
    ...(ADXml.abstractDocOfXml(
      ADXml.creators(imageResources1, {}, ADXml.extractImageFontsStyleNames(xml)[2]),
      xml[0]!
