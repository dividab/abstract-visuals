...
export const LINE = (CARTESIAN_POINT: number, VECTOR: number, m: MutableStep): number => mutate(`LINE('',#${CARTESIAN_POINT},#${VECTOR})`, m);
...
export const EDGE_CURVE = (startVertex: number, endVertex: number, line: number, m: MutableStep, type: "T" | "F" = "F"): number =>
  mutate(`EDGE_CURVE('',#${startVertex},#${endVertex},#${line},.${type}.)`, m);
...
export const OPEN_SHELL = (ADVANCED_FACE: number, m: MutableStep): number => mutate(`OPEN_SHELL('',(#${ADVANCED_FACE}))`, m);

export const FACE_BOUND = (EDGE_LOOP: number, type: "T" | "F", m: MutableStep): number => mutate(`FACE_BOUND('',#${EDGE_LOOP},.${type}.)`, m);
...
export const PLANE = (AXIS2_PLACEMENT_3D: number, m: MutableStep): number => mutate(`PLANE('',#${AXIS2_PLACEMENT_3D})`, m);

export const PCURVE = (CYLINDRICAL_SURFACE_or_PLANE: number, DEFINITIONAL_REPRESENTATION: number, m: MutableStep): number =>
  mutate(`PCURVE('',#${CYLINDRICAL_SURFACE_or_PLANE},#${DEFINITIONAL_REPRESENTATION})`, m);
...
export const SHELL_BASED_SURFACE_MODEL = (OPEN_SHELL: number, m: MutableStep): number => mutate(`SHELL_BASED_SURFACE_MODEL('',(#${OPEN_SHELL}))`, m);
...
export const CLOSED_SHELL2 = (ADVANCED_FACE: number, m: MutableStep): number => mutate(`CLOSED_SHELL('',(#${ADVANCED_FACE}))`, m);

export const MANIFOLD_SOLID_BREP = (closedRef: number, m: MutableStep): number => mutate(`MANIFOLD_SOLID_BREP('',#${closedRef})`, m);

export const AXIS2_PLACEMENT_3D = (CARTESIAN_POINT: number, DIRECTION_NORMAL: number, DIRECTION_PLANE_DIRECITON: number, m: MutableStep): number =>
  mutate(`AXIS2_PLACEMENT_3D('',#${CARTESIAN_POINT},#${DIRECTION_NORMAL},#${DIRECTION_PLANE_DIRECITON})`, m);
...
export const STYLED_ITEM = (PRESENTATION_STYLE_ASSIGNMENT: number, MANIFOLD_SOLID_BREP: number, m: MutableStep): number =>
  mutate(`STYLED_ITEM('color',(#${PRESENTATION_STYLE_ASSIGNMENT}),#${MANIFOLD_SOLID_BREP})`, m);

export const PRESENTATION_STYLE_ASSIGNMENT = (SURFACE_STYLE_USAGE: number, CURVE_STYLE: number, m: MutableStep): number =>
  mutate(`PRESENTATION_STYLE_ASSIGNMENT((#${SURFACE_STYLE_USAGE},#${CURVE_STYLE}))`, m);
...
export const SURFACE_STYLE_FILL_AREA = (FILL_AREA_STYLE: number, m: MutableStep): number => mutate(`SURFACE_STYLE_FILL_AREA(#${FILL_AREA_STYLE})`, m);
...
export const FILL_AREA_STYLE_COLOUR = (COLOUR_RGB: number, m: MutableStep): number => mutate(`FILL_AREA_STYLE_COLOUR('',#${COLOUR_RGB})`, m);
...
export const LENGTH_UNIT = (m: MutableStep): number => mutate(`( LENGTH_UNIT() NAMED_UNIT(*) SI_UNIT(.MILLI.,.METRE.) )`, m);

export const NAMED_UNIT = (unit: "PLANE_ANGLE_UNIT() SI_UNIT($,.RADIAN.)" | "SI_UNIT($,.STERADIAN.) SOLID_ANGLE_UNIT()", m: MutableStep): number =>
  mutate(`( NAMED_UNIT(*) ${unit} )`, m);
...
export const SHAPE_DEFINITION_REPRESENTATION = (productDefinitionShape: number, manifoldSurfaceShapeRepr: number, m: MutableStep): number =>
  mutate(`SHAPE_DEFINITION_REPRESENTATION(#${productDefinitionShape},#${manifoldSurfaceShapeRepr})`, m);
...
export const PRODUCT_DEFINITION = (productDefinitionFormation: number, productDefinitionContext: number, m: MutableStep): number =>
  mutate(`PRODUCT_DEFINITION('design',#${productDefinitionFormation},#${productDefinitionContext})`, m);

export const PRODUCT_DEFINITION_FORMATION = (product: number, m: MutableStep): number => mutate(`PRODUCT_DEFINITION_FORMATION('','',#${product})`, m);
