import type { Material, Vec3, Cylinder } from "../../../abstract-3d.js";
import { vec3TransRot, vec3RotCombine, vec3Zero, vec3, vec3Add, vec3Rot } from "../../../abstract-3d.js";
...
export function stepCylinder(
  ...
  const isCylinder = Math.abs(requestedTopRadius - bottomRadius) <= radiusTolerance;

  const topRadius = isCylinder ? bottomRadius : requestedTopRadius;
  ...
  const bottomCenter = vec3Add(pos, rot(vec3(0, -h / 2, 0)));

  const topCenter = vec3Add(pos, rot(vec3(0, h / 2, 0)));

  const bottomSeamPoint = vec3Add(pos, rot(vec3(bottomRadius, -h / 2, 0)));
  ...
  const seamDirection = vec3(seamDelta.x / seamLength, seamDelta.y / seamLength, seamDelta.z / seamLength);
  ...
  const bottomCenterPoint = CARTESIAN_POINT(bottomCenter, m);

  const topCenterPoint = CARTESIAN_POINT(topCenter, m);

  const bottomSeamCartesianPoint = CARTESIAN_POINT(bottomSeamPoint, m);

  const topSeamCartesianPoint = CARTESIAN_POINT(topSeamPoint, m);

  const bottomVertex = VERTEX_POINT(bottomSeamCartesianPoint, m);

  const topVertex = VERTEX_POINT(topSeamCartesianPoint, m);

  const bottomPlacement = AXIS2_PLACEMENT_3D(bottomCenterPoint, axisDirection, radialDirection, m);
  ...
  const topCircleEdge = EDGE_CURVE(topVertex, topVertex, CIRCLE(topPlacement, topRadius, m), m, "T");

  const growsTowardTop = topRadius >= bottomRadius;

  const coneAxis = growsTowardTop ? axis : vec3(-axis.x, -axis.y, -axis.z);

  const coneLocation = growsTowardTop ? bottomCenterPoint : topCenterPoint;

  const coneStartRadius = Math.min(bottomRadius, topRadius);

  const semiAngle = Math.atan2(Math.abs(topRadius - bottomRadius), h);

  const conePlacement = AXIS2_PLACEMENT_3D(coneLocation, DIRECTION(coneAxis, m), radialDirection, m);

  const sideSurface = isCylinder
    ? CYLINDRICAL_SURFACE(bottomPlacement, bottomRadius, m)
    : CONICAL_SURFACE(conePlacement, coneStartRadius, semiAngle, m);
