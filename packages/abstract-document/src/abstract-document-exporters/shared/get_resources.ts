...
function getResourcesSection(s: AD.Section.Section): AD.Resources.Resources {
  const header = AD.Resources.mergeResources(
    s.page.header.map(e => getResourcesSectionElement(e))
  );
  const footer = AD.Resources.mergeResources(
    s.page.footer.map(e => getResourcesSectionElement(e))
  );
  const children = AD.Resources.mergeResources(
    s.children.map(e => getResourcesSectionElement(e))
  );
  return AD.Resources.mergeResources([header, footer, children]);
}

function getResourcesSectionElement(
...
function getResourcesParagraph(
...
function getResourcesTable(table: AD.Table.Table): AD.Resources.Resources {
  return AD.Resources.mergeResources(
    table.children.map(r => getResourcesTableRow(r))
  );
}

function getResourcesTableRow(r: AD.TableRow.TableRow): AD.Resources.Resources {
  return AD.Resources.mergeResources(
    r.children.map(c => getResourcesTableCell(c))
  );
}

function getResourcesTableCell(
  c: AD.TableCell.TableCell
): AD.Resources.Resources {
  return AD.Resources.mergeResources(
    c.children.map(e => getResourcesSectionElement(e))
  );
}

function getResourcesGroup(group: AD.Group.Group): AD.Resources.Resources {
  return AD.Resources.mergeResources(
    group.children.map(e => getResourcesSectionElement(e))
  );
}
