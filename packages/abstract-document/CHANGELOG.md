...
- Fixed: numeric template attributes (`width`, `fontSize`, `rowSpan`, etc.) now reject `Infinity`/`-Infinity` in addition to non-numeric text, falling back to `0` instead of letting a non-finite number reach layout code.
- Internal: activated `typescript/no-unnecessary-condition`. No other functional changes.
